...
This module identifies and recognizes data structures like arrays and structs
in the decompiled code to improve variable identification and code readability.
"""
...
class DataStructureRecognizer:
    ...
    def analyze(self) -> bool:
        ...
        """
        try:
            logger.info("Starting data structure recognition")
            ...
            logger.info(f"Found {len(self.arrays)} arrays and {len(self.structs)} structs")
            
            self.analysis_complete = True
            return True
            
        except Exception as e:
            logger.error(f"Error in data structure analysis: {str(e)}")
            return False
    ...
    def _collect_memory_accesses(self):
        ...
        logger.info(f"Collected memory accesses for {len(self.data_accesses)} addresses")
    ...
    def _identify_arrays(self):
        ...
        logger.info(f"Identified {len(self.arrays)} arrays")
    ...
    def _identify_adjacent_arrays(self):
        ...
        logger.info(f"Identified adjacent arrays and 2D arrays, resulting in {len(self.arrays)} arrays")
    ...
    def _identify_structs(self):
        ...
        logger.info(f"Identified {len(self.structs)} structs")
    ...
    def _determine_struct_address(self, base_reg: str, fields: Dict[int, List[X86Instruction]]) -> Optional[int]:
        ...
        # If we can't determine the address, generate a placeholder
        return None
    ...
    def _enhance_functions(self):
        ...
        logger.info("Enhanced functions with data structure information")
    ...
    def generate_structure_report(self) -> str:
        ...
        if not self.analysis_complete:
            logger.warning("Data structure analysis not complete, running analyze() first")
            self.analyze()
        ...
        if self.arrays:
            ...
            for addr, array in sorted(self.arrays.items()):
                if hasattr(array, "is_2d") and array.is_2d:
                    report.append(f"  2D Array: {array.element_type} {array.name}[{array.rows}][{array.cols}] at 0x{array.address:X}")
                else:
                    report.append(f"  Array: {array.element_type} {array.name}[{array.length}] at 0x{array.address:X}")
                report.append(f"    Size: {array.size} bytes")
                report.append(f"    Element Size: {array.element_size} bytes")
                if array.comment:
                    report.append(f"    Comment: {array.comment}")
                report.append("")
        ...
        if self.structs:
            ...
            for addr, struct in sorted(self.structs.items()):
                ...
                report.append(f"    Size: {struct.size} bytes")
                report.append("    Fields:")
                for offset, name, field_type, size in sorted(struct.fields, key=lambda x: x[0]):
                    report.append(f"      {field_type} {name} at offset {offset} (size: {size} bytes)")
                report.append("")
        
        # Report on functions that use data structures
        struct_users = {}
        array_users = {}
        
        for func in self.functions:
            if hasattr(func, "struct_defs") and func.struct_defs:
                struct_names = [struct.name for struct in func.struct_defs.values()]
                if struct_names:
                    struct_users[func.name] = struct_names
            
            if hasattr(func, "variables"):
                array_vars = [var for var in func.variables.values() if var.is_array]
                if array_vars:
                    array_users[func.name] = [var.name for var in array_vars]
        
        if struct_users:
            report.append("Functions Using Structs:")
            for func_name, structs in sorted(struct_users.items()):
                report.append(f"  {func_name}: {', '.join(structs)}")
            report.append("")
        
        if array_users:
            report.append("Functions Using Arrays:")
            for func_name, arrays in sorted(array_users.items()):
                report.append(f"  {func_name}: {', '.join(arrays)}")
            report.append("")
